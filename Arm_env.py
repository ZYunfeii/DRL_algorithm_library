...
if __name__ == "__main__":
    ...
    # sys.path.append('./TD3/TD3_spinningup')
    # from TD3.TD3_spinningup.td3 import TD3
    # model = TD3(env.state_dim, env.action_dim)
    # model.ac.load_state_dict(torch.load('./TD3/TrainedModel/td3_ac.pkl'))

    sys.path.append('./PPO/multi_processing_ppo')
    from PPO.multi_processing_ppo.PPOModel import *
    net = GlobalNet(env.state_dim,env.action_dim)
    net.act.load_state_dict(torch.load('./PPO/TrainedModel/act.pkl'))
    while 1:
        env.render()
